export const BASE_URL = "http://192.168.0.106"; // Updated to current local IP for physical device testing
...
export const ENDPOINTS = {
  ...
  ADMIN_ORDER_DETAILS: (id: string) =>
    `${API_BASE}/admin-api/order-details/${id}`,
  ...
  ADMIN_BOOKING_DETAILS: (id: string) =>
    `${API_BASE}/admin-api/booking-details/${id}`,
  ADMIN_GET_ROOM_TYPES: `${API_BASE}/admin-api/get-room-types`,
  ADMIN_GET_AVAILABLE_ROOMS: (typeId: string) =>
    `${API_BASE}/admin-api/get-available-rooms?room_type=${typeId}`,
  ...
  ADMIN_BOOKING_INVOICE: (id: string) =>
    `${API_BASE}/admin-api/booking-invoice/${id}`,
